...
document.addEventListener("livewire:init", () => {
    Livewire.on("notify", (e) => {
        ...
        toast.innerText = e.message || "Berhasil";
...
function buildLineDataset(title, values, color) {
    return {
        label: title,
        data: values,
        tension: 0.4,
...
function applyChartPayload(payload) {
    ...
    if (!chart.data.datasets.length) {
        chart.data.datasets = [buildLineDataset(title, values, color)];
    } else {
...
function renderMetricChart(payload) {
    ...
    window.__robMetricChart = new Chart(canvas.getContext("2d"), {
        ...
        data: {
            labels,
            datasets: [buildLineDataset(title, values, color)],
        },
        options: {
            responsive: true,
...
window.addEventListener("modalChart", (e) => {
    renderMetricChart(e.detail || {});
});
    ...
    Alpine.data("dashboard", (liveTheme) => ({
        ...
        init() {
            ...
            this.$watch("theme", (value) => {
                ...
                if (window.__robMainChart) {
                    applyChartPayload({
                        labels: window.__robMainChart.data.labels,
                        values: window.__robMainChart.data.datasets?.[0]?.data ?? [],
                        title: window.__robMainChart.data.datasets?.[0]?.label ?? "Grafik Sensor",
                        metric: "ketinggian_air",
                    });
                }

                if (window.__robMetricChart && window.__robMetricCanvas) {
                    const chart = window.__robMetricChart;
                    const dataset = chart.data.datasets?.[0];

                    renderMetricChart({
                        labels: chart.data.labels ?? [],
                        values: dataset?.data ?? [],
                        title: dataset?.label ?? "Metric",
                    });
                }
            });
        },
    ...
    Alpine.data("windyMapComponent", (cfg) => ({
        ...
        async init() {
            ...
            if (!this.key || this.key.trim() === "") {
                this.error =
                    "Windy API Key belum dikonfigurasi di file .env (WINDY_KEY)";
                this.loading = false;
                console.error("Windy Key is empty");
                return;
            }

            if (this.__initAttempt > MAX_WINDY_INIT_ATTEMPTS) {
                this.error =
                    "Gagal initialize Windy setelah beberapa percobaan. Silakan refresh halaman.";
                this.loading = false;
                return;
            }
            ...
            if (window.__windyMap && window.__windyReady) {
                this.map = window.__windyMap;
                this.markersLayer = window.__windyMarkers;

                if (!this.markersLayer && window.L) {
            ...
            const vw =
                (rect.width > 0 ? rect.width : prect.width) || window.innerWidth;
            const vh =
            ...
            try {
                const windyAPI = await new Promise((resolve, reject) => {
                    const timeoutId = setTimeout(() => {
                        reject(
                            new Error(
                                "Windy init timeout (10 detik). Cek koneksi internet atau pastikan API Key valid.",
                            ),
                        );
                    }, 10000);
                ...
                this.error =
                    err.message ||
                    "Gagal memuat peta Windy. Cek Console untuk detail.";
                this.loading = false;
            }
        },
        ...
        _ensureMapReady() {
            const windyEl = document.getElementById("windy");
            if (!windyEl || !this.map) return;

            windyEl.classList.remove("hidden", "free-model");
            windyEl.style.removeProperty("display");

            if (this._vw && this._vh) {
                windyEl.style.width = `${this._vw}px`;
                windyEl.style.height = `${this._vh}px`;
            } else {
                windyEl.style.width = "100%";
                windyEl.style.height = "100%";
            }
        ...
        _bindResize() {
            ...
            setTimeout(this.__resizeHandler, 50);
        },
        ...
        _watchHidden() {
            ...
            this._observer = new MutationObserver(() => {
                ...
                if (this._vw && this._vh) {
                    el.style.width = `${this._vw}px`;
                    el.style.height = `${this._vh}px`;
                }
...
document.addEventListener("alpine:init", () => {
    Alpine.data("analisisChart", () => ({
        ...
        renderCharts() {
            ...
            const sensorLabels = sensorData.map((d) =>
                String(d.local_datetime || "").substring(11, 16),
            );
            const bmkgLabels = bmkgData.map((d) =>
                String(d.local_datetime || "").substring(11, 16),
            );
                ...
                this.charts[id] = new Chart(canvas.getContext("2d"), {
                    ...
                    data: {
                        ...
                        datasets: [
                            makeDataset(
                                "Sensor",
                                sensorData.map((d) => d[key]),
                                color,
                            ),
                            makeDataset(
                                "BMKG",
                                bmkgData.map((d) => d[key]),
                                "#94a3b8",
                                true,
                            ),
                        ],
