...
function applyChartPayload(payload) {
    ...
    if (!chart) { window.__robChartPending = p; return; }
    ...
    if (!chart.data.datasets.length) {
        chart.data.datasets = [{
            label: title, data: [], tension: 0.4, fill: true,
            borderWidth: 2, borderColor: color.border, backgroundColor: color.bg,
            pointRadius: 3, pointHoverRadius: 4,
            pointBackgroundColor: color.border, pointBorderColor: color.border,
            pointHoverBackgroundColor: color.border, pointHoverBorderColor: color.border,
        }];
    }

    const ds = chart.data.datasets[0];
    ds.label = title; ds.data = values;
    ds.borderColor = color.border; ds.backgroundColor = color.bg;
    ds.pointRadius = 3; ds.pointHoverRadius = 4;
    ds.pointBackgroundColor = color.border; ds.pointBorderColor = color.border;
    ds.pointHoverBackgroundColor = color.border; ds.pointHoverBorderColor = color.border;
    chart.update("none");
}
...
function renderMetricChart(payload) {
    ...
    if (!canvas) { window.__robMetricPending = payload; return; }
    ...
    if (window.__robMetricChart) { window.__robMetricChart.destroy(); window.__robMetricChart = null; }
    ...
    window.__robMetricChart = new Chart(canvas.getContext("2d"), {
        ...
        data: {
            labels,
            datasets: [{
                label: title, data: values, tension: 0.4, fill: true,
                borderWidth: 2, borderColor: color.border, backgroundColor: color.bg,
                pointRadius: 3, pointHoverRadius: 4,
                pointBackgroundColor: color.border, pointBorderColor: color.border,
                pointHoverBackgroundColor: color.border, pointHoverBorderColor: "#ffffff",
            }],
        },
        options: {
            ...
            scales: {
                x: { ticks: { maxTicksLimit: 8, font: { size: 11 } }, grid: { color: "rgba(255,255,255,0.05)" } },
...
document.addEventListener("alpine:init", () => {
    Alpine.data("dashboard", (liveTheme) => ({
        data: {}, theme: liveTheme ?? "dark",
        risk: "AMAN",
        riskStyles: { bg: "bg-emerald-500/10", border: "border-emerald-500/30", text: "text-emerald-600" },
        ...
        init() {
            ...
            this.$nextTick(() => {
                ...
                if (!window.__robMainChart) {
                    window.__robMainChart = new Chart(canvas.getContext("2d"), {
                        ...
                        data: {
                            labels: [],
                            datasets: [{
                                label: "Grafik Sensor", data: [], tension: 0.4, fill: true,
                                ...
                                pointBackgroundColor:     SENSOR_COLORS.ketinggian_air.border,
                                pointBorderColor:         SENSOR_COLORS.ketinggian_air.border,
                                pointHoverBackgroundColor:SENSOR_COLORS.ketinggian_air.border,
                ...
                if (window.__robChartPending) { applyChartPayload(window.__robChartPending); window.__robChartPending = null; }
            });

            window.addEventListener("dashboard-updated", (e) => {
                this.data       = e.detail?.data       || {};
                this.risk       = e.detail?.risk       || "AMAN";
                this.riskStyles = e.detail?.riskStyles || { bg: "bg-emerald-500/10", border: "border-emerald-500/30", text: "text-emerald-600" };
            });
...
document.addEventListener("alpine:init", () => {
    Alpine.data("windyMapComponent", (cfg) => ({
        ...
        map: null, store: null, markersLayer: null,
        loading: true, error: "",
        activeOverlay: cfg?.overlay ?? "wind",
        ...
        async init() {
            if (window.__windyMap) {
                ...
                this.store        = window.__windyStore;
            ...
            if (!container) { this.error = "Container #windy tidak ditemukan."; return; }

            try {
                await loadScriptOnce("https://api.windy.com/assets/map-forecast/libBoot.js");
            } catch (e) {
                this.error = "Gagal load Windy API. Cek koneksi internet."; return;
            }
            ...
            const windyReady = await new Promise((resolve) => {
                ...
                const iv  = setInterval(() => {
                    tries++;
                    if (typeof window.windyInit === "function") { clearInterval(iv); resolve(true); }
                    else if (tries > 100)                        { clearInterval(iv); resolve(false); }
                }, 100);
            });

            if (!windyReady) { this.error = "Windy API timeout. Cek API key atau koneksi."; return; }

            const rect  = container.getBoundingClientRect();
            const prect = container.parentElement?.getBoundingClientRect() ?? rect;
            ...
            window.windyInit(
                { key: this.key, lat: this.lat, lon: this.lon, zoom: this.zoom, overlay: this.overlay, verbose: false },
                (windyAPI) => {
                    this.map   = windyAPI.map;
                    this.store = windyAPI.store;
                    window.__windyMap   = this.map;
                    window.__windyStore = this.store;
        ...
        // ── Ganti layer — langsung set, tanpa cek availableOverlays ──────────
        changeOverlay(layer) {
            ...
            try {
                ...
                console.warn(`[Windy] Layer "${layer}" tidak didukung:`, e.message);
            }
        },
        ...
        fitToDevices(devices) {
            ...
            const doFit = () => {
                try {
                    if (pts.length === 1) { this.map.setView(pts[0], 13, { animate: false }); return; }
                    this.map.fitBounds(LLeaflet.latLngBounds(pts), { padding: [50, 50], animate: false });
                } catch (e) {
                    setTimeout(() => {
                        try { this.map.fitBounds(LLeaflet.latLngBounds(pts), { padding: [50, 50], animate: false }); } catch (_) {}
                    }, 150);
                }
            };
        ...
        _invalidateSoon() {
            [0, 100, 300, 600].forEach((ms) => {
                setTimeout(() => { try { this.map?.invalidateSize(true); } catch (_) {} }, ms);
            });
        },
        ...
        _bindResize() {
            ...
            const handler = () => {
                ...
                if (rect.width > 0 && rect.height > 0) {
                    this._vw = rect.width; this._vh = rect.height;
        ...
        _watchHidden(vw, vh) {
            ...
            this._observer.observe(windyEl, { attributes: true, attributeFilter: ["class", "style"] });
        },
        ...
        renderMarkers(devices) {
            ...
            (devices || []).forEach((d) => {
                ...
                const isOnline = (d.status || "offline").toLowerCase() === "online";
                const risiko   = d.status_risiko ?? "UNKNOWN";

                const risikoColor = { AMAN: "#22c55e", WASPADA: "#f59e0b", SIAGA: "#f97316", BAHAYA: "#ef4444", UNKNOWN: "#94a3b8" }[risiko] ?? "#94a3b8";
                const risikoBg    = { AMAN: "#f0fdf4", WASPADA: "#fefce8", SIAGA: "#fff7ed", BAHAYA: "#fef2f2", UNKNOWN: "#f8fafc"  }[risiko] ?? "#f8fafc";

                const icon = LLeaflet.divIcon({
                    className: "",
                    html: `<div style="width:38px;height:38px;background:${risikoColor};border:3px solid white;border-radius:50%;box-shadow:0 2px 12px rgba(0,0,0,0.25);display:flex;align-items:center;justify-content:center;">
                ...
                const sensorHtml = s ? `
                    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-top:10px;">
                        <div style="background:#f0f9ff;border-radius:10px;padding:8px;text-align:center;">
                            <div style="font-size:9px;color:#0284c7;font-weight:600;text-transform:uppercase;">Ketinggian</div>
                        ...
                        <div style="background:#fff7ed;border-radius:10px;padding:8px;text-align:center;">
                            <div style="font-size:9px;color:#ea580c;font-weight:600;text-transform:uppercase;">Suhu</div>
                        ...
                        <div style="background:#ecfeff;border-radius:10px;padding:8px;text-align:center;">
                            <div style="font-size:9px;color:#0891b2;font-weight:600;text-transform:uppercase;">Kelembapan</div>
                        ...
                        <div style="background:#f0fdf4;border-radius:10px;padding:8px;text-align:center;">
                            <div style="font-size:9px;color:#16a34a;font-weight:600;text-transform:uppercase;">Tekanan</div>
                        ...
                        <div style="background:#fffbeb;border-radius:10px;padding:8px;text-align:center;">
                            <div style="font-size:9px;color:#d97706;font-weight:600;text-transform:uppercase;">Angin</div>
                        ...
                        <div style="background:#faf5ff;border-radius:10px;padding:8px;text-align:center;">
                            <div style="font-size:9px;color:#9333ea;font-weight:600;text-transform:uppercase;">Arah Angin</div>
                ...
                ` : `<div style="text-align:center;padding:16px 0;color:#94a3b8;font-size:12px;">Tidak ada data sensor</div>`;
                ...
                marker.bindPopup(`
                    <div style="font-family:'Bricolage Grotesque',ui-sans-serif,sans-serif;min-width:240px;max-width:280px;">
                        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:8px;">
                            ...
                            <span style="background:${risikoBg};color:${risikoColor};font-size:10px;font-weight:700;padding:3px 10px;border-radius:999px;border:1px solid ${risikoColor}30;white-space:nowrap;margin-left:8px;flex-shrink:0;">${risiko}</span>
                        </div>
...
document.addEventListener("alpine:init", () => {
    Alpine.data("searchableDeviceSelect", (config = {}) => ({
        open: false, query: "", selected: config.selected || null, options: config.options || [],

        init() { this.$watch("selected", (value) => { if (config.onChange) config.onChange(value); }); },
        ...
        get selectedOption() { return this.options.find((item) => String(item.id) === String(this.selected)) || null; },

        select(item) { this.selected = item.id; this.query = ""; this.open = false; },
...
document.addEventListener("alpine:init", () => {
    Alpine.data("searchSelect", (config = {}) => ({
        ...
        init() { this.$watch("isOpen", (open) => { if (open) this.$nextTick(() => this.$refs.search?.focus()); }); },

        options() {
            if (typeof this.getOptions === "function") { const r = this.getOptions(); return Array.isArray(r) ? r : []; }
        ...
        selectedOption() { return this.options().find((opt) => String(opt.value) === String(this.value)) || null; },
        selectedLabel()  { return this.selectedOption()?.label ?? this.placeholder; },
        toggle() { this.isOpen = !this.isOpen; },
        close()  { this.isOpen = false; this.query = ""; },
        select(val) { this.value = val; this.isOpen = false; this.query = ""; },
    }));
});
...
document.addEventListener("alpine:init", () => {
    Alpine.data("analisisChart", () => ({
        ...
        init() {
            ...
            window.addEventListener("livewire:navigated", () => this.$nextTick(() => this.renderCharts()));
            document.addEventListener("livewire:update",  () => this.$nextTick(() => this.renderCharts()));
        },

        destroyAll() { Object.values(this.charts).forEach((c) => { try { c.destroy(); } catch (_) {} }); this.charts = {}; },
