                ...
                @foreach ($navLinks as $link)
                    <a href="{{ route($link['route']) }}"
                        @class([
                            ...
                            'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-semibold' => request()->routeIs($link['route']),
                            'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800' => !request()->routeIs($link['route']),
                        ])>
                        {{ $link['label'] }}
                    </a>
                @endforeach
            ...
            @foreach ($navLinks as $link)
                <a href="{{ route($link['route']) }}"
                    ...
                    @class([
                        ...
                        'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-semibold' => request()->routeIs($link['route']),
                        'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800' => !request()->routeIs($link['route']),
                    ])>
                    {{ $link['label'] }}
                </a>
            @endforeach
