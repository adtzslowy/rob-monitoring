    ...
    <div class="mx-auto space-y-4 sm:space-y-5">
                        ...
                        @forelse($devices as $d)
                            @php
                                $lastUpdate = $d['timestamp']
                                    ? \Illuminate\Support\Carbon::parse($d['timestamp'])->timezone('Asia/Jakarta')->format('d M H:i')
                                    : '-';
                            @endphp
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition">
                                ...
                                <td class="px-3 sm:px-5 py-3 sm:py-4 text-left">
                                    ...
                                    <div class="text-xs text-zinc-500 dark:text-zinc-400 truncate">
                                        ID: {{ $d['id'] }} @if(!empty($d['name']) && $d['alias'] && $d['alias'] !== $d['name']) • {{ $d['name'] }} @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-3 sm:px-5 py-3 sm:py-4 text-center">
                                    <span
                                        ...
                                        <span class="inline-flex h-2 w-2 rounded-full {{ $d['online'] ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                                        {{ $d['online'] ? 'Online' : 'Offline' }}
                                    </span>
                                </td>

                                <td class="px-3 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap text-zinc-700 dark:text-zinc-200">
                                ...
                                <td class="px-3 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap text-zinc-700 dark:text-zinc-200">
                                    @if($d['arah_angin'] !== null)
                                        {{ $d['arah_angin'] }}° <span class="text-zinc-500">({{ $d['arah_angin_label'] }})</span>
                                    @else
                                        -
                                    @endif
                                </td>
                                ...
                                <td class="px-3 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap text-zinc-600 dark:text-zinc-300">
                                    {{ $lastUpdate }}
                                </td>
        ...
        @if ($modalOpen && $this->detailDevice)
            ...
            <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
                ...
                <div class="relative w-full sm:max-w-3xl mx-0 sm:mx-4 rounded-t-2xl sm:rounded-2xl border border-zinc-200 dark:border-zinc-800
                    ...
                    <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div class="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/70 dark:bg-zinc-900/40 px-4 py-3">
                        ...
                        <div class="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/70 dark:bg-zinc-900/40 px-4 py-3">
                            ...
                            <div class="mt-1 text-sm font-medium {{ $isOnline ? 'text-emerald-600 dark:text-emerald-300' : 'text-red-600 dark:text-red-300' }}">
                        ...
                        <table class="min-w-full w-full text-sm border-collapse">
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-zinc-900 dark:text-zinc-100">{{ $detailReading?->suhu !== null ? $detailReading->suhu . ' °C' : '-' }}</td>
                                </tr>
                                <tr>
                                    <td class="px-4 py-3 font-medium text-zinc-600 dark:text-zinc-400">Kelembapan</td>
                                    <td class="px-4 py-3 text-zinc-900 dark:text-zinc-100">{{ $detailReading?->kelembapan !== null ? $detailReading->kelembapan . ' %' : '-' }}</td>
                                </tr>
                                <tr>
                                    <td class="px-4 py-3 font-medium text-zinc-600 dark:text-zinc-400">Tekanan Udara</td>
                                    <td class="px-4 py-3 text-zinc-900 dark:text-zinc-100">{{ $detailReading?->tekanan_udara !== null ? $detailReading->tekanan_udara . ' hPa' : '-' }}</td>
                                </tr>
                                <tr>
                                    <td class="px-4 py-3 font-medium text-zinc-600 dark:text-zinc-400">Kecepatan Angin</td>
                                    <td class="px-4 py-3 text-zinc-900 dark:text-zinc-100">{{ $detailReading?->kecepatan_angin !== null ? $detailReading->kecepatan_angin . ' m/s' : '-' }}</td>
                                </tr>
                                <tr>
                                    <td class="px-4 py-3 font-medium text-zinc-600 dark:text-zinc-400">Arah Angin</td>
                                    <td class="px-4 py-3 text-zinc-900 dark:text-zinc-100">
                                        @if($detailReading?->arah_angin !== null)
                                            {{ $detailReading->arah_angin }}° ({{ $this->getWindDirectionLabel($detailReading->arah_angin) }})
                                        @else
                                            -
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td class="px-4 py-3 font-medium text-zinc-600 dark:text-zinc-400">Ketinggian Air</td>
                                    <td class="px-4 py-3 text-zinc-900 dark:text-zinc-100">{{ $detailReading?->ketinggian_air !== null ? $detailReading->ketinggian_air . ' cm' : '-' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
